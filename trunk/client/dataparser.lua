...
-- 解析顺序表
local function parseOrder(fullFileName)
			...
			for i=1, fieldCount do
				...
				elseif "list_number" == fieldType then
					if "" == value or "nil" == value then
						...
						rowStr = rowStr..serialize(stringSplit(value, "|", true))
					end
				elseif "list_string" == fieldType then
					if "" == value or "nil" == value then
						rowStr = rowStr.."{}"
					else
						rowStr = rowStr..serialize(stringSplit(value, "|", false))
					end
				elseif "tuple_number" == fieldType then
					if "" == value or "nil" == value then
						rowStr = rowStr.."{}"
					else
						rowStr = rowStr..serialize(parseTuple(value, "|", true))
					end
				elseif "tuple_string" == fieldType then
					if "" == value or "nil" == value then
						rowStr = rowStr.."{}"
					else
						rowStr = rowStr..serialize(parseTuple(value, "|", false))
					end
				else
...
-- 解析哈希表
local function parseHash(fullFileName)
	local dataTable = dofile(fullFileName)
	assert("table" == type(dataTable), "file "..fullFileName.." is not table format")
	local row, keyTable, dataStr = 0, {}, "return{"
...
local function run(path)
	...
	for key, fullFileName in pairs(tb) do
		local dataTable = dofile(fullFileName)
		if "table" == type(dataTable) and "table" == type(dataTable[1]) then
			if "string" == type(dataTable[1][1]) then
				parseOrder(fullFileName)
			else
				parseHash(fullFileName)
			end
		end
	end
end
