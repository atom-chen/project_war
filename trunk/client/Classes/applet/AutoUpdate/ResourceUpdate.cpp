...
//------------------------------------------------------------------------
void ResourceUpdate::setNative(const std::string& path,const std::string& nativeVersionFile, const std::string& nativeMd5File)
{
...
//------------------------------------------------------------------------
void ResourceUpdate::checkVersion(const std::string& url, const std::string& checkVersionFile, const std::string& checkMd5File)
{
...
//------------------------------------------------------------------------
void ResourceUpdate::checkUpdate(bool deepCheck /*= true*/)
{
...
void ResourceUpdate::handleMd5CheckFileDownloaded(bool downloaded, const std::string& fileURL, int curlCode, int responseCode, const std::string& errorBuffer)
{
	if (downloaded)
	{
		parseFileInfo(mDownloadDir + mCheckVersionFile, mCheckInfoVec);
		createUpdatePthread();
	}
	else
	{
...
void ResourceUpdate::updateImpl(void)
{
	...
	{
		if (!mDeepCheckUpdate)	// 非深度检查,不保存新版本号
		{
