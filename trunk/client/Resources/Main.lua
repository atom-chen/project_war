...
----------------------------------------------------------------------
G = G or {}
G.PLATFORM = cc.Application:getInstance():getTargetPlatform()	-- target platform type
...
globalcall(function()
	...
	gameScene:runAction(cc.Sequence:create(cc.DelayTime:create(0.05), cc.CallFunc:create(function()
		...
		G.APPVERSION = cc.UserDefault:getInstance():getStringForKey("OriginVersion")
		-- check run flow
		...
		ChannelProxy:login(function()
			local updateURL = ""
			if cc.PLATFORM_OS_ANDROID == G.PLATFORM or 
			   cc.PLATFORM_OS_IPHONE == G.PLATFORM or cc.PLATFORM_OS_IPAD == G.PLATFORM or 
			   cc.PLATFORM_OS_WP8 == G.PLATFORM then
				-- updateURL = G.CONFIG["update_url"]
			end
			require("AutoUpdate")
			AutoUpdate_start(updateURL, function()
				loadConfig()
				require("Game")
				Game:init()
			end, G.CONFIG["debug"])
