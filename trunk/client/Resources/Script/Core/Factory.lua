...
----------------------------------------------------------------------
Factory = {
	mNormalIdList = {2001, 2002, 2003, 2004, 2005},		-- 普通元素id列表
}
...
function Factory:getNormalId(normalType)
	return self.mNormalIdList[normalType]
end

-- 是否为普通元素
function Factory:isNormalId(normalId)
	for normalType, id in pairs(self.mNormalIdList) do
		if normalId == id then
			return true
		end
	end
