...
-- 是否在坐标列表正周围
function Core:isAroundCoordList(coord, coordList, boardDatas)
	for key, val in pairs(coordList) do
		if (coord.row == val.row or coord.col == val.col) and Core:isCanContact(coord, val, boardDatas) then
			return true
		end
	end
	return false
