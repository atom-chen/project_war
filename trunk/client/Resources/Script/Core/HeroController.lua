...
function HeroController:ctor()
	...
	self:bind(EventDef["ED_CLEAR_GRID"], self.handleClearGrid, self)
	self:bind(EventDef["ED_DROP_END"], self.handleDropEnd, self)
end
...
function HeroController:searchThrowCoord(skillElement)
	...
	for row=2, rowCount do
		...
		for col=1, colCount do
			...
			if grid and nil == grid:getFixedElement() and nil == grid:getCoverElement()
				and ElementType["normal"] == grid:getShowElement():getType() and not Core:isCoordExist(self.mThrowCoordList, grid:getCoord()) then
				if skillElement:getExtraType() == grid:getShowElement():getSubType() then
...
function HeroController:setSkillGrid(skillElement, setCF)
	...
	Core:addCoord(self.mThrowCoordList, coord)
	...
	Actions:bezierTo(skillElement:getSprite(), cfg, 0.25, function()
		...
		local gridController = self:getSibling("GridController")
		local _, grid = gridController:getGrid(coord.row, coord.col)
		grid:destroyElements()
		grid:setElement(Factory:createSkillElement(skillElement:getSkillId()))
		gridController:setGrid(coord.row, coord.col, {skillElement:getData().id}, grid)
...
function HeroController:showSkillSprites(normalCountList)
	...
	for normalType, count in pairs(normalCountList) do
		...
		if hero and count > 0 then
			hero:showSkillRound()
			hero:showSkillAppendRound(count)
		end
	end
end
...
function HeroController:handleTouchGridBegin(touchParam)
	local hero = self.mHeroTable[touchParam.touched_type]
	if hero then
		hero:playPreAttack()
	end
...
-- 当前出场英雄中攻击力最高的普通元素
function HeroController:getMaxAttackNormalId()
	-- 筛选最高攻击力
	local normalIdList, maxAttack = {}, 0
	for normalType=ElementNormalType["red"], ElementNormalType["purple"] do
		local attack = G.NORMAL_BASE_ATTACK
		local hero = self.mHeroTable[normalType]
		if hero then
			attack = hero:getData().attack
		end
		if attack > maxAttack then
			maxAttack = attack
			normalIdList = {}
			table.insert(normalIdList, Factory:getNormalId(normalType))
		elseif attack == maxAttack then
			table.insert(normalIdList, Factory:getNormalId(normalType))
		end
	end
	return CommonFunc:getRandom(normalIdList) or 0
end
...
function HeroController:createHeroInfoPanel()
	...
	-- 普通元素
	for i=ElementNormalType["red"], ElementNormalType["purple"] do
		...
		if hero then
			...
			-- 当前进度
			local currProgress = cc.ProgressTimer:create(cc.Sprite:create("gray_02.png"))
			currProgress:setType(cc.PROGRESS_TIMER_TYPE_RADIAL)
			currProgress:setReverseDirection(true)
			currProgress:setPosition(cc.p(xPos, topHeight - 38))
			currProgress:setScale(0.78)
			infoPanel:addChild(currProgress)
			self.mHeroProgressTable[i] = currProgress
			-- 英雄勋章
...
-- 设置是否可显示英雄详细信息
function HeroController:setShowHeroDetail(isShow)
	self.mIsShowHeroDetail = isShow
end

-- 显示英雄信息面板
function HeroController:showHeroInfoPanel()
	if not self.mIsShowHeroDetail then
		return
	end
	for normalType, hero in pairs(self.mHeroTable) do
		local currProgress = self.mHeroProgressTable[normalType]
		currProgress:setPercentage((1 - hero:getSkillCurrRound()/hero:getSkillTriggerRound())*100)
	end
...
function HeroController:heroAttack(elementList, hero)
	...
	local monster = monsterController:getMonster()
	...
	for i, element in pairs(elementList) do
		...
		Actions:delayWith(element:getSprite(), 0.5 + (i - 1)*0.05, function()
			Actions:bezierTo(element:getSprite(), bezierCfg, moveDuration, function()
				...
				monsterController:onHurt(element, remainCount)
			end)
			if 1 == i then
...
function HeroController:elementAttack(elementType, normalType, id, coord, index, flyCF)
	...
	Actions:delayWith(tempElement:getSprite(), (index - 1)*0.1 + 0.1, function()
		...
		Actions:delayWith(tempElement:getSprite(), 0.1, function()
			...
			Actions:bezierTo(tempElement:getSprite(), bezierCfg, moveDuration, function()
				Utils:doCallback(flyCF)
				monsterController:onHurt(tempElement, index)
			end)
		end)
	end)
	return true
