...
-- Brief: 四格漫画界面
----------------------------------------------------------------------
UIDEFINE("UICG", "Cg.csb")
function UICG:onStart(ui, param)
	local cgDatas = LogicTable:getAll("cg_tplt")
	local cgIndex = 1
	-- cg面板
	self.mCgPanel = self:getChild("Panel_root")
	self:addPanel(cgDatas[1].image, 200)
	self:addTouchEvent(self.mCgPanel, function(sender)
		cclog("enter***************",cgIndex)
		...
		local action = cc.Spawn:create(cc.FadeOut:create(1), cc.CallFunc:create(function()
			self:addPanel(cgDatas[cgIndex].image, 100)
			if cgIndex == 3 then
