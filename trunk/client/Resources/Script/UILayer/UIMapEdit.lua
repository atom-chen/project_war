...
-- Brief: 地图编辑器界面
----------------------------------------------------------------------
UIDEFINE("UIMapEdit", "MapEdit.csb")
...
function UIMapEdit:onStart(ui, param)
	mRoot = ui.root
	mGridPanel = self:getChild("Panel_3")
	UIMapEditElementData:initElementData()
	--保存的文件名
	mTextField = self:getChild("fileName")
	--初始化数据
	self:initMapUI()
	
	-- 保存按钮
	local saveBtn = self:getChild("Button_save")
	self:addTouchEvent(saveBtn, function(sender)
		self:saveBtnClick(sender)
	end, true, true, 0)
	
	--导入按钮
	local importBtn = self:getChild("import")
	self:addTouchEvent(importBtn, function(sender)
...
function UIMapEdit:setUIByImportData(tempData)
	for i =1,8,1 do
		for j= 1,11,1 do
			local grid = mGridPanel:getChildByName(string.format("grid_%d_%d",i,j))
			local cover = grid:getChildByName(string.format("cover_%d_%d",i,j))
			local fix = grid:getChildByName(string.format("fixed_%d_%d",i,j))
			local block_up = mGridPanel:getChildByName(string.format("block_H_%d_%d",i,j))
			local block_right = mGridPanel:getChildByName(string.format("block_V_%d_%d",i,j+1))
			local block_down = mGridPanel:getChildByName(string.format("block_H_%d_%d",i+1,j))
			local block_left = mGridPanel:getChildByName(string.format("block_V_%d_%d",i,j))
...
function UIMapEdit:loadIconBySelectedType(sender)
	...
	if elementIconStr == UIMapEditElementData:getElementIconById(speElement["ink"]) or
	   ...
	   elementIconStr == UIMapEditElementData:getElementIconById(speElement["ink_2"]) then
		...
		local grid = mGridPanel:getChildByName(string.format("grid%s",str))
		local cover = grid:getChildByName(name)
	...
	elseif  elementIconStr == UIMapEditElementData:getElementIconById(speElement["fence"])  then --"fixed_4_5"
		...
		local grid = mGridPanel:getChildByName(string.format("grid%s",str))
		local fix = grid:getChildByName(name)
		fix:loadTexture(elementIconStr)
		UIMapEditGridData:setGridDataByElement(mCheckBoxIndex,row_i,row_j,"")	
	elseif  elementIconStr == "ImageFile.png" then --空的元素
		local str = string.sub(sender:getName(),6,-1)
		local name =string.format("grid%s",str)
		local grid = mGridPanel:getChildByName(name)
		grid:loadTexture(elementIconStr)
		local cover = grid:getChildByName(string.format("cover%s",str))
		local fix = grid:getChildByName(string.format("fixed%s",str))
...
function UIMapEdit:cellClick(sender)
	...
	if  string.sub(senderName,1,4) == "grid"  and (UIMapEditElementData:judgeBloc(imgStr) == true)  then
		UIPrompt:show("此种类型只能用于阻碍，不能用于格子")
		return
	end
	
	if UIMapEditElementData:judgeBloc(imgStr) == false and (string.sub(senderName,1,4) == "bloc")then
		UIPrompt:show("此种类型只能用于格子，不能用于阻碍")
		return
	end
	
	local i,j,direct = 0,0,""
	local arr = CommonFunc:stringSplit(senderName, "_", false)
	...
	local grid = mGridPanel:getChildByName("grid_"..i.."_"..j)
	local cover = grid:getChildByName("cover_"..i.."_"..j)
	local fix = grid:getChildByName("fixed_"..i.."_"..j)
...
local function checkBoxClick(sender,eventType)
	if eventType == ccui.CheckBoxEventType.selected then
		...
		if mCheckBoxIndex ~= 0 then
			...
			local lastSelctBox = UIMapEdit:getChild(name)
...
function UIMapEdit:saveBtnClick(sender)
	if mTextField:getString() == "" then
		...
		--存文件为csv格式
		local fileName = self:getChild("fileName")
		...
		--初始化数据
		self:initMapUI()
		UIPrompt:show("保存成功，请在client/csv目录下查看")
	end
end
--------------------------------------------------------------------------------------------------
--导入按钮触发的事件
...
function UIMapEdit:initScrollCheckBox()
	mScrollView = self:getChild("ScrollView_2") --"ScrollView_item"
	...
	UIScrollViewEx.show(mScrollView, data, createCell,"V", 130, 70, 2,4, 50, false, nil, true, false)
end
--------------------------------------------------------------------------------------------------
--初始化界面的一些数据和布局
...
function UIMapEdit:initEmptyCell()
	...
	for i=1,8,1 do
		for j = 1,11,1 do
			...
			self:addTouchEvent(grid,  function(sender)
	...
	for i=1,8,1 do
		for j = 1,12,1 do
			...
			local blockV = mGridPanel:getChildByName(name)
			...
			self:addTouchEvent(blockV, function(sender)
				self:cellClick(sender)
			end, true, true, 0)
		end
	end
	...
	for i=1,9,1 do
		for j = 1,11,1 do
			...
			local blockH = mGridPanel:getChildByName(name)
			...
			self:addTouchEvent(blockH,  function(sender)
				self:cellClick(sender)
			end, true, true, 0)
		end
	end
...
function UIMapEdit:addCoverImg(widget)
	...
	--点击覆盖类触发函数	
	self:addTouchEvent(imageView, function(sender)
