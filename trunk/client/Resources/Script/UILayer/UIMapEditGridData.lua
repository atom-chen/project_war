...
function UIMapEditGridData:importToCSV(inputWidget)
	...
	local allGridData = Utils:loadCsvFile("../data/"..gridName..".csv")
	local allBoardData = Utils:loadCsvFile("../data/"..boardName..".csv")
	--根据导入的数据设置数据表
...
--------------------------------------------------------------------------------------------------
--写入csv格式文件
function UIMapEditGridData:saveToCSV(fileSuffix)
	-- 格子数据
	local gridDataString = ""
	for i=1, 11 do
		if i > 1 then
			gridDataString = gridDataString..","
		end
		gridDataString = gridDataString..string.format("list_number:%d", i)
	end
	for i=1, 8 do
		gridDataString = gridDataString.."\n"
		for j=1, 11 do
			...
			gridDataString = gridDataString..self:getGridDataByCoord(i, j)
		end
	end
	CommonFunc:writeFile(string.format("../data/grid_%s.csv", fileSuffix), gridDataString)
	-- 隔板数据
	local boardDataString = ""
	for i=1, 12 do
		if i > 1 then
			boardDataString = boardDataString..","
		end
		boardDataString = boardDataString..string.format("list_number:%d", i)
	end
	for i=1, 9 do
		boardDataString = boardDataString.."\n"
		for j=1, 12 do
			...
			boardDataString = boardDataString..self:getGridAroundDataByCoord(i, j)
		end
	end
	CommonFunc:writeFile(string.format("../data/board_%s.csv", fileSuffix), boardDataString)
end
