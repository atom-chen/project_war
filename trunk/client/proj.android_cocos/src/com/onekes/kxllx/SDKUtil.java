...
public final class SDKUtil {
	...
	***************************************************************************************/
	...
	public static void onCreate(Activity activity) {
	    ...
	    // cocos sdk
	    CocosPayApi.getInstance().initialize(activity, new CocosPayCallback() {
            @Override
            public void onInitStart() {
                // cocospay框架开始初始化，建议写应用的自己的初始化
            	Log.e("cocos2d", "cocosPay -> onInitStart " );
            }
            
            @Override
            public void onInitFinished() {
                // cocospay框架初始化完成，此时方可调用cocospay框架的支付能力
            	Log.e("cocos2d", "cocosPay -> onInitFinished ");
            	mOperatorType = String.valueOf(CocosPayApi.getInstance().getReportSdkType());
            }
        });
	    // umeng update
	...
	***************************************************************************************/
	...
	public static String get(int type, String data) {
		if (1 == type) {	// 获取运营商类型
			...
			return CocosPayApi.getInstance().isSoundOn() ? "true" : "false";
		}
		return "";
	}
	//-------------------------------------------------------------------------------------
	...
	public static void logout(SDKListener listener) {
		...
		.setPositiveButton(R.string.onekes_queding, new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface arg0, int arg1) {
				CocosPayApi.getInstance().exit(mActivity, new CocosPayExitCallback() {
		            @Override
		            public void onExit() {
	...
	public static void showAbout(String data) {
		CocosPayApi.getInstance().showAbout(data);
	}
	/***************************************************************************************
	******************************** private module
	***************************************************************************************/
	...
	private static void handlePay(SDKListener listener) {
		...
		try {
			...
			String ckcode = "";
			if (jsonObj.has("ckcode")) {
				ckcode = jsonObj.getString("ckcode");
			}
			CocosPayApi.getInstance().doPayment(mActivity, ckcode, new PaymentCallback() {
				public void paySuccess(PaymentResult result) {
					// 支付成功
					Log.e("cocos2d", "pay -> paySuccess -> sdkType: " + result.getSdkType());
					mPayListener.onCallback(1, null);
					mHandler.sendEmptyMessage(MT_PAY_SUCCESS);
				}
				public void payFailed(PaymentResult result) {
		         	// 支付失败
					Log.e("cocos2d", "pay -> payFailed -> sdkType: " + result.getSdkType() + ", reason: " +result.getReason() + ", errorCode: " +result.getErrorCode());
					mPayListener.onCallback(2, null);
					mHandler.sendEmptyMessage(MT_PAY_FAIL);
				}
				public void payCanceled(PaymentResult result) {
					// 用户取消的支付
					Log.e("cocos2d", "pay -> payCanceled -> sdkType: " + result.getSdkType());
					mPayListener.onCallback(3, null);
					mHandler.sendEmptyMessage(MT_PAY_CANCEL);
				}			
			});
		} catch(Exception e) {
			e.printStackTrace();
