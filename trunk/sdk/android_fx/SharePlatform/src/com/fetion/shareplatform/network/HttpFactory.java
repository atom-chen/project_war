...
public class HttpFactory {
	...
	public static HttpResponse execute(Context context, HttpGet get)
			throws IOException {
		if (!Utils.isNetworkAvaliable(context) && isWapNetwork()) {
			setWapProxy();
		...
		try {
			...
			if (statusCode == HttpStatus.SC_OK) {
				...
				if (entity != null) {
					...
					String str = Utils.stream2String(in);
			...
			Log.i("NetHelper", "__getHttpData__" + e.toString()
					+ "_____");
		}
		return "";
	}
	...
	public static String postHttpData(Context context, String url,
		...
		try {
			...
			post.abort();
			Log.i("NetHelper", "__postHttpData__" + ex.toString()
					+ "_____");
		} finally {
			if (entity != null) {
				try {
					...
					Log.i("NetHelper", "__postHttpData__" + e.toString()
							+ "_____");
				}
			}
