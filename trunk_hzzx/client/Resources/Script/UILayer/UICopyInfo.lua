...
function UICopyInfo:addPanel()
	...
	levelText:setColor(cc.c3b(61,41,41))
		...
		if unlockHeroId[i] ~= nil then		--加载一页或多页
			...
			local function createCell(page, ItemData, index)
				...
				page:addTouchEventListener( function( _sender, _type )	
					--cclog(_type,_sender:getName(),Log(pageView:getInnerContainer():getPosition()))
					...
					elseif _type == ccui.TouchEventType.moved  then
						...
						if 0 == offset then 
							--cclog("----moved**ItemData.heroIndex,index**********",ItemData.heroIndex,index)
						...
						--cclog("================= move---realIndex,ItemData.heroIndex-- : "..realIndex,ItemData.heroIndex)
						if realIndex <= 1 then
							realIndex = 1
					...
					elseif _type == ccui.TouchEventType.ended or _type == ccui.TouchEventType.canceled then
						--Log("抬起时**",pageView:getInnerContainer():getPosition())
						--Log(pageView:getInnerContainer():getContentSize())
						...
						--cclog("================= ended---realIndex,ItemData.heroIndex : "..realIndex,ItemData.heroIndex)
						--滑到对应的位置
...
--设置消耗体力
function UICopyInfo:setPower()
	print("UICopyInfo:setPower()***********",self.power,CopyModel:getCostHp())
...
function UICopyInfo:onStart(ui, param)
	...
	Utils:addTouchEvent(btnRestart, function(sender)
		if DataLevelInfo:canEnterCopy() == false then
			...
			UIManager:openFront(UIFreePower,true)		-- 打开购买体力界面
			return
		end
		DataHeroInfo:getSelectHeroId()
	...
	--设置消耗体力
	self.power = UIManager:seekNodeByName(ui.root, "Text_power")
	self.power:setString(CopyModel:getCostHp())
	self:loadGoals()
