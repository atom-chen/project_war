...
-- 投放到格子
function ThrowController:throwToGrid(throwElementIds, throwCF)
	...
	local xPos, yPos = self:getSibling("MonsterController"):getMonster():getNode():getPosition()
	local monsterDisplay = self:getSibling("MonsterController"):getMonster():getDisplay()
...
function ThrowController:searchWetland(triggerGridList, validCoordList, boardDatas)
	...
	for key, grid in pairs(wetlandGridList) do
		for k, validCoord in pairs(validCoordList) do
			if Core:isCanContact(grid:getCoord(), validCoord, boardDatas) then
