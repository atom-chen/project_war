...
function Monster:ctor(monsterId)
	...
	-- 创建怪物血条精灵
	self.mBloodSprite = cc.Sprite:create("blood_02.png")
	local glProgramState = Utils:createShader(self.mBloodSprite, "common.vsh", "show_hp.fsh")
	glProgramState:setUniformFloatEx("fHave", self.mMaxHP/self.mMaxHP)
	glProgramState:setUniformFloatEx("fWill", self.mMaxHP/self.mMaxHP)
	self.mBloodSprite:setGLProgramState(glProgramState)
	-- 创建怪物当前血条文本
...
function Monster:destroy()
	...
	if self.mBloodSprite then
		self.mBloodSprite:removeFromParent()
		self.mBloodSprite = nil
	end
...
function Monster:deadEffect(effectCF)
	...
	self.mBloodSprite:setVisible(false)
	if self.mDefenseBgSprite then
		self.mDefenseBgSprite:setVisible(false)
	end
	if self.mDefenseSprite then
...
function Monster:bornEffect()
	Actions:fadeIn(self.mMonsterNode, 0.5, function()
		...
		self.mBloodSprite:setVisible(true)
		if self.mDefenseBgSprite then
...
-- 获取怪物血条精灵
function Monster:getBloodSprite()
	return self.mBloodSprite
end
...
-- 更新怪物生命值
function Monster:updateHP(hitBlood)
	...
	self.mBloodSprite:getGLProgramState():setUniformFloatEx("fHave", hpPercent)
	return true
end
...
function Monster:showHaveHp()
	...
	self.mBloodSprite:getGLProgramState():setUniformFloatEx("fHave", currPercent)
	-- 血条文本
...
function Monster:showWillHP(hitBlood)
	...
	self.mBloodSprite:getGLProgramState():setUniformFloatEx("fWill", willPercent)
	-- 血条文本
...
function Monster:resetWillHP()
	self.mBloodSprite:getGLProgramState():setUniformFloatEx("fWill", self.mCurrHP/self.mMaxHP)
	self.mWillBloodLabelBMFont:setVisible(false)
end
