...
function Component:addSibling(com)
	...
	if self.mMaster then	-- 不可添加宿主为同级组件
		local masterName = self.mMaster:getName()
		assert(comName ~= masterName, "can't add master '"..comName.."' to be sibling")
	end
...
-- 注册事件函数(handler:必须是类函数)
function Component:subscribeEvent(eventId, handler, priority)
	assert(nil == self.mEventHandlerMap[eventId], "a component can't subscribe two same event")
	self.mEventHandlerMap[eventId] = function(param)
		handler(self, param)
	end
	EventDispatcher:subscribe(eventId, self.mEventHandlerMap[eventId], priority)
end
