...
function MapManager:create(copyId, heroIds)
	...
	if copyInfo.resource_id > 0 then
		...
		if copyResourceInfo.seabed_effect[1] then
			...
			if copyResourceInfo.seabed_effect[2] then
				local glProgramState = Utils:createShader(seabedEffect, "common.vsh", copyResourceInfo.seabed_effect[2])
				glProgramState:setUniformFloatEx("timer", 0)
				local timer = 0
				seabedEffect:runAction(cc.RepeatForever:create(cc.Sequence:create({
					...
					cc.CallFunc:create(function()
						...
						glProgramState:setUniformFloatEx("timer", timer)
					end)
				})))
			end
		end
