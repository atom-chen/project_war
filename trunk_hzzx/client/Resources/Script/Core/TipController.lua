...
-- 显示对话框
function TipController:showDialog(textString)
	if self.mGrayBgSprite or self.mDialogNode then
		return
	end
	self:setTouchSwallow(true)
	self.mGrayBgSprite = GuideUI:createGrayBg()
	self:getMaster():getTopLayer():addChild(self.mGrayBgSprite, G.TOP_ZORDER_GUIDE)
	self.mDialogNode = GuideUI:createDialog(textString)
	self:getMaster():getTopLayer():addChild(self.mDialogNode, G.TOP_ZORDER_GUIDE)
end
...
function TipController:parsetGuideStep()
	local opType, opValue, exValue = GuideMgr:getCopyStep()
	if nil == opType then
		return false
	end
	if 1 == opType then
		self:showDialog(opValue)
