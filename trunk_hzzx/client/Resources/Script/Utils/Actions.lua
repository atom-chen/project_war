...
function Actions:moveScaleAction01(node, pos)
	if nil == node then return end
	node:runAction(cc.Sequence:create(cc.ScaleTo:create(0.4, 3), cc.ScaleTo:create(0.6, 0.5)))
	node:runAction(cc.Sequence:create(cc.MoveTo:create(1, cc.vertex2F(pos.x, pos.y)), cc.CallFunc:create(function()
		node:removeFromParent()
	end)))
end
----------------------------------------------------------------------
