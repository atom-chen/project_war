...
local mMustShowHeroAwardIds = {5004,5001,5010,5013}	-- 抽奖必现的奖励id
local mMustShowHeroIds = {2101,1101,4101,5101}		-- 抽奖必现的英雄id
local mMustShowHeroLevels = {5,7,10,13}			-- 抽奖必现的英雄等级
...
function DataLevelInfo:getRandomOneId(awardPondInfo)
	...
	local pro = Probability.new(proTb)
	return pro:getValue()
end
...
function  DataLevelInfo:showDiscountMoves()
	if DataLevelInfo:getFailTimes() == G.DIS_MOVES_TIMES and tostring(ChannelPayCode:getBuyDisMovPrice()) ~= "0"then
		...
		UIManager:openFront(UIDiscountMoves,true)
	end
end
...
function DataLevelInfo:showDisCountDiamond()
	if  DataLevelInfo:getOpenDisDiaFlag() then
...
function DataLevelInfo:setDisDiaRandomTb()
	local channelTb = DataLevelInfo:getChannelData()
	local randomTb = {}
	local phoneSimState = ChannelProxy:getPhoneSimState()	--获得手机是什么卡
	local stateStr = ""
	--测试--phoneSimState = "SIM_YD" --"SIM_LT"	--"SIM_YD"	--"SIM_NULL"	--"SIM_UNKNOWN"
	if phoneSimState == "SIM_NULL" or phoneSimState == "SIM_UNKNOWN" then
		return channelTb
	elseif phoneSimState == "SIM_DX" then
		stateStr = "DISCOUNT_CIAMOND_TY_CODE"
	elseif phoneSimState == "SIM_LT" then
		stateStr = "DISCOUNT_CIAMOND_LT_CODE"
	elseif phoneSimState == "SIM_YD" then
		stateStr = "DISCOUNT_CIAMOND_YD_CODE"
	end
	
	for key,val in pairs(channelTb) do
		for k,v in pairs(val) do
			if stateStr == k and tostring(0) ~= v then
				table.insert(randomTb,val)
			end
		end
	end
