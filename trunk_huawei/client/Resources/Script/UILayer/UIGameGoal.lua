...
function UIGameGoal:updateLeftMoves()
	...
	if leftMoves == copyInfo.moves - 1 and (UIBuyMoves:getBuyMovesFlag() == false)  then
...
function UIGameGoal:checkGameOver(isCanConnect)
	...
	if completeFlag and currMoves >= 0 then		-- 目标完成
		...
		DataLevelInfo:setOpenDisDiaFlag(self.mIsFirstSuccess)	-- 设置是否显示砖石打折界面
	...
	elseif not completeFlag then 				-- 目标未完成
		if isCanConnect then	-- 有可连接的元素
			if currMoves <= 0 then		-- 步数用完
				...
				-- 购买步数界面
				ChannelProxy:getStrInfo("buy_moves", function(productTb)
					if 0 == #productTb then
						--UIPrompt:show(LanguageStr("QURERY_FAIL"))
						UIManager:close(self)
						UIMiddlePub:openUIFailed()
					else
						UIManager:openFront(UIBuyMoves,true,{["product_tb"]=productTb})
					end
				end)
...
function UIGameGoal:nodeFlyToTarget()
	local goals = CopyModel:getGoals()
	for i=1, 3 do
		local back = UIManager:seekNodeByName(self.mRootNode, "back_"..i)			--左上角的底板
		local iconBg = UIManager:seekNodeByName(self.mRootNode, "ImageBg_"..i)		--中间目标的底板
		local icon = UIManager:seekNodeByName(self.mRootNode, "Icon_"..i)			--中间目标的图片
		local image = UIManager:seekNodeByName(self.mRootNode, "Image_"..i)			--左上角的目标
		local amount = UIManager:seekNodeByName(self.mRootNode, "number_"..i)		--左上角的个数
