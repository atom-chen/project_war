...
function UIGetAward:onStart(ui, param)
	...
	self.Button_buyKey = UIManager:seekNodeByName(ui.root, "Button_buyKey")
	...
	Utils:addTouchEvent(self.Button_buyKey, function(sender)
		...
		if curDiamond > needDiamond then
			...
			ItemModel:appendTotalDiamond(-needDiamond)
			GetRewardModel:updateRewardInfo()
			...
			ChannelProxy:getStrInfo("shop", function(productTb)
				if 0 == #productTb then
					...
					UIManager:openFront(UIBuyDiamond,true,{["enter_mode"] ="buy",["diamondNumber"] = needDiamond - curDiamond, ["product_tb"]=productTb})
				end
			end)
