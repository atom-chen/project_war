...
function MonsterController:init(monsterIds)
	...
	self:getMaster():getSceneLayer():addChild(comboLabelBMFont, G.SCENE_ZORDER_TIP)
...
function MonsterController:onHurt(hitElement, index)
	...
	if self.mIsStartHurt then
		...
		-- 
		Actions:shakeAction04(self:getMaster():getSceneLayer())
		-- 怪物被击音效
...
function MonsterController:handleClearBegin()
	...
	if self.mMultiple > 1 then
		...
		local praiseImg = ccui.ImageView:create()
		praiseImg:loadTexture(multipleInfo.praise_tip)	
		praiseImg:setAnchorPoint(cc.p(0.5,0.5))
		praiseImg:setPosition(cc.p(G.VISIBLE_SIZE.width/2, 450))
		praiseImg:setOpacity(0)
		--[[
		...
		self:getMaster():getTopLayer():addChild(praiseImg, G.TOP_ZORDER_TIP)
		Actions:moveScaleAction01(praiseImg, cc.p(G.VISIBLE_SIZE.width/2, 350))
