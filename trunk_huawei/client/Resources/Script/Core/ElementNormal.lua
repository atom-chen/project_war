...
function ElementNormal:clearAction(param, actionEndCF)
	local data = self:getData()
	if MapManager:getComponent("HeroController"):flyToHero(data.type, data.sub_type, data.id, param.coord, param.index, actionEndCF) then
		return
	end
	if MapManager:getComponent("HeroController"):elementAttack(data.type, data.sub_type, data.id, param.coord, param.index, actionEndCF) then
		return
	end
