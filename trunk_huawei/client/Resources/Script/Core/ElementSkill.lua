...
function ElementSkill:clearAction(param, actionEndCF)
	local data = self:getData()
	if MapManager:getComponent("HeroController"):flyToHero(data.type, data.extra_type, self.mSkillId, param.coord, param.index, actionEndCF) then
		return
	end
	if MapManager:getComponent("HeroController"):elementAttack(data.type, data.extra_type, self.mSkillId, param.coord, param.index, actionEndCF) then
		return
	end
