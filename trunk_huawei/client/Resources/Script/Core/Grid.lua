...
function Grid:setCoord(row, col)
	...
	self.mIsBorn = self.mController:isBornCoord({row = self.mRow, col = self.mCol})
end
...
-- 检查元素是否为空
function Grid:checkEmptyElement(element)
...
-- 生成下一个元素
function Grid:generateNextElement(element, nextElementId)
		...
		if nextElementId > 0 then
			...
			if isForce or self.mController:isPassiveCoord(self:getCoord()) then
				...
				if nil == nextElementId then	-- 没有不可消除的元素
					self:checkEmptyElement(element)
					return
				end
			end
			self:generateNextElement(element, nextElementId)
		else
			self:checkEmptyElement(element)
		end
	end, function()
...
function Grid:onReset(resetCoordPath, resetCF)
	...
	Actions:moveToWithPath(self.mNode, pathArray, 0.2, resetCF)
end
